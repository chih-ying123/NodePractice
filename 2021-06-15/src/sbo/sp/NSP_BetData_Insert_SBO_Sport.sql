...
DROP PROCEDURE IF EXISTS `NSP_BetData_Insert_SBO_Sport`$$

CREATE DEFINER=`winnie`@`192.168.202.12` PROCEDURE `NSP_BetData_Insert_SBO_Sport`(
	_refNo VARCHAR(32)
    , _username VARCHAR(50)
    , _sportsType VARCHAR(32)
    , _orderTime DATETIME
    ...
    , _isHalfWonLose TINYINT
    , _isLive TINYINT
    , _maxWinWithoutActualStake DECIMAL(20,10)
    , _ip VARCHAR(32)
    , _IsSystemTagRisky TINYINT(1) 
    , _isCustomerTagRisky TINYINT(1) 
    , _subBet JSON
    , _BetInfo TEXT
)
main:BEGIN
...
DECLARE _GameVendor         INT               DEFAULT 45  ; -- 第三方平台編號 select * from Game_Vendor ;
DECLARE _GroupId         	INT               DEFAULT 71  ; -- 第三方平台編號 select * from Game_Vendor ;
...
IF NOT EXISTS (SELECT 1 FROM Game_List WHERE GameVendor = _GameVendor AND TheirGameId = _sportsType)
THEN 
		
	INSERT INTO Game_List
	SET
		GameVendor      = _GameVendor           
		, TheirGameId   = _sportsType
		, GameName      = _sportsType
		, GameEName     = _sportsType
		, GameType      = 1
		, FanShuiType   = 'T' 
		, Note          = ''
	;
...
IF NOT EXISTS(SELECT 1 FROM betdata_sbo_sport WHERE refNo = _refNo)
THEN
	...
	, modifyDate = _modifyDate 
        ...
        , winLost = _winLost 
        , turnover = _turnover 
        , isHalfWonLose = _isHalfWonLose 
        , isLive = _isLive 
        , maxWinWithoutActualStake = _maxWinWithoutActualStake 
        , ip = _ip
        , IsSystemTagRisky = _IsSystemTagRisky
	, isCustomerTagRisky = _isCustomerTagRisky
	, subBet  = _subBet 
	, betInfo  = _betInfo 
	;
ELSE

	SELECT `status`,modifyDate INTO _status_old,_modifyDate_old FROM betdata_sbo_sport WHERE refNo = _refNo;
...
IF NOT EXISTS(SELECT 1 FROM report_winlose_detail WHERE GameVendor = _GameVendor AND ZhuDanId = _refNo)
THEN

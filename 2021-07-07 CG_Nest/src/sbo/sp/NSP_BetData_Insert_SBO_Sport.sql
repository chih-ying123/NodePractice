USE `platform_db_wrc` ; -- 暫時使用 ( 正式版本需移除或是修改為 platform_db )
...
IF NOT EXISTS (SELECT 1 FROM Game_List WHERE GameVendor = _GameVendor AND TheirGameId = _sportType)
THEN 
		
	INSERT INTO Game_List
	SET
		GameVendor      = _GameVendor           
		, TheirGameId   = _sportType
		, GameName      = _sportType
		, GameEName      = _sportType
		, GameType      = 1
		, FanShuiType   = 'T' 
		, Note          = ''
	;

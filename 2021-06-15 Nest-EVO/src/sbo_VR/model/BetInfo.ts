...
export class BetInfo {
    ...
    public static init() {
        BetInfo.Template_BetInfo = fs.readFileSync("./src/sbo_VR/views/template_betinfo.html","utf8");
    }

    public static ToHtml(bet: IPlayerBetList) {
        var itemList = [];
        ...
        //console.log(dist);
        if(!fs.existsSync("./src/sbo_VR/betinfo")) {
            fs.mkdir("./src/sbo_VR/betinfo", { recursive: true }, (err) => {
                if (err) throw err;
            });
        }

        let output = Common.renderHtml(BetInfo.Template_BetInfo,  dist);
        fs.writeFileSync("./src/sbo_VR/betinfo/"+bet.refNo+".html", output);
        return output;
    }
}
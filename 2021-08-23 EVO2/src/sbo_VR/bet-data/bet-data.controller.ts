...
@Controller('wrin/sbo_VR/bet-data')
export class BetDataController {
    constructor(private service: sbo_VRService) { }

    @Get()
    public index(): Promise<string> {
        return Common.readHtml('./html/sbo_VR.html');
    }
  ...
  public async CallBySchedule(@Req() req, @Param() date) {
    ...
    console.log("sbo_VR: schedule:", date );
  ...
  public async CallByDate(@Param('date') date: string) {
    ...
    //   if (sbo_VRService.Active) return "busy";
    //   sbo_VRService.Active = true;
